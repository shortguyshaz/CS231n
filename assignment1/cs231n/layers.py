...
def svm_loss(x, y):
    ...
    margin = np.maximum(0, x - correct_scores + 1)
    margin[np.arange(num_train), y] = 0
    loss = margin.sum() / num_train

    dx = np.zeros(x.shape)
    valid = np.sum(margin > 0, axis=1)
    dx[margin > 0] = 1
    dx[np.arange(num_train), y] -= valid
    dx /= num_train
...
def softmax_loss(x, y):
    ...
    exp = np.sum(np.exp(temp), axis=1, keepdims=True)
    softmax = np.exp(temp) / exp
